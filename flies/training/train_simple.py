"""
Training script for VQ-VAE with SIMPLE initialization fix (no normalization).

This uses VQVAESimple which only fixes the codebook initialization.
No pre-quantizer GroupNorm layer. Closer to original VQ-VAE paper.

Try this FIRST! If VQ loss is still high (>50), use train.py with GroupNorm instead.

Usage:
    python train_simple.py --train_data /path/to/data.npy \
...
from vq_vae.vqvae_simple import VQVAESimple as VQVAE  # Use simple version!
